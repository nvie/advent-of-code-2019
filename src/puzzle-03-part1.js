...
// Coords get encoded as "x,y", e.g. "0,0", or "2,3".  This is to make them
// a single, primitive structure, so we can put them in a Set.
export type Coord = string;
export type XY = {| x: number, y: number |};

export function toXY(coord: Coord): XY {
...
/**
 * Takes a "full path" input, like "R8,U3,L5" and returns all coordinates that
 * this path covers, starting from the starting point.
 */
export function path(path: string): Set<Coord> {
  const sections = path.split(',');
  const result = new Set();
  let start = '0,0';
  for (const sec of sections) {
    const pts = section(sec, start);
    for (const pt of pts) {
      result.add(pt);
    }
...
function distance(coord: Coord): number {
  const { x, y } = toXY(coord);
  return Math.abs(x) + Math.abs(y); // Manhattan distance
}
...
if (require.main === module) {
  ...
  const distances = crossings.map(distance);
